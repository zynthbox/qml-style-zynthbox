...
T.Slider {
    ...
    background: PlasmaCore.FrameSvgItem {
        ...
        PlasmaCore.FrameSvgItem {
            ...
            width: control.horizontal ? Math.max(fixedMargins.left + fixedMargins.right, Math.round(control.position * (control.availableWidth - (control.handle ? control.handle.width : 0) / 2) + ((control.handle ? control.handle.width : 0)  / 2))) : parent.width
            height: control.vertical ? Math.max(fixedMargins.top + fixedMargins.bottom, Math.round(control.position * (control.availableHeight - (control.handle ? control.handle.height : 0) / 2) + ((control.handle ? control.handle.height : 0) / 2))) : parent.height
        }
        ...
        Loader {
            ...
            width: control.horizontal ? parent.width - (control.handle ? control.handle.width : 0) : (control.background ? control.background.x : 0)
            height: control.vertical ? parent.height - (control.handle ? control.handle.height : 0) : (control.background ? control.background.y : 0)
